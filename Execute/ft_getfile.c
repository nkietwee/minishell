...
/*   Updated: 2023/10/07 16:49:55 by pnamwayk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_infile(t_list *tb_lst)
{
	...
	while (rdr_lst)
	{
		...
		{
			table->fd_in = open(rdr->file, O_RDONLY);
			dprintf(2, "rdr->file : %s\n" , rdr->file);
			if (i != table->nbr_infile - 1)
			{
				close(table->fd_in);
				if (rdr->type == HEREDOC ) \
					unlink(rdr->file);
...
void	get_outfile(t_list *tb_lst)
{
	...
	while (rdr_lst)
	{
		...
		{
			if (rdr->type == APPEND)
				table->fd_out = open(rdr->file, O_WRONLY | O_CREAT | O_APPEND, 0644);
			if (rdr->type == OUTFILE )
				table->fd_out = open(rdr->file, O_WRONLY | O_CREAT | O_TRUNC, 0644);
			if (i != table->nbr_outfile - 1)
				close(table->fd_out);
			i++;
		}
		rdr_lst = rdr_lst->next;
